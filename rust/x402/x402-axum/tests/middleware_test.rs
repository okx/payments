...
// ---------------------------------------------------------------------------
// ResourceInfo.url priority: `route_config.resource` > middleware-extracted.
// Tests cover both branches end-to-end through the 402 response body.
// ---------------------------------------------------------------------------

/// Build an app where /weather declares an explicit `resource` URL override.
async fn build_app_with_resource_override(
    mock_server: &MockServer,
    resource: Option<String>,
) -> Router {
    mount_supported_mock(mock_server).await;
    let facilitator =
        OkxHttpFacilitatorClient::with_url(&mock_server.uri(), "key", "secret", "pass").unwrap();
    let mut server =
        X402ResourceServer::new(facilitator).register("eip155:196", ExactEvmScheme::new());
    server.initialize().await.expect("initialize failed");
...
/// Extract the `resource.url` field from a 402 JSON response body.
async fn read_resource_url(resp: axum::response::Response) -> String {
    ...
    json["resource"]["url"]
        .as_str()
        .expect("resource.url is a string")
        .to_string()
}

#[tokio::test]
async fn resource_url_uses_route_config_override_when_set() {
    let mock_server = MockServer::start().await;
    let app =
        build_app_with_resource_override(&mock_server, Some("https://canonical.example.com/v1/weather".into()))
            .await;
    ...
    let response = app
        .oneshot(
            Request::builder()
                ...
                // Even with a Host header present, the explicit `resource`
                // override wins.
                .header("host", "internal-svc:8080")
                .header("x-forwarded-proto", "https")
                .header("x-forwarded-host", "api.shop.com")
                .body(Body::empty())
                .unwrap(),
        )
        .await
    ...
    assert_eq!(response.status(), StatusCode::PAYMENT_REQUIRED);
    assert_eq!(
        read_resource_url(response).await,
        "https://canonical.example.com/v1/weather"
    );
}

#[tokio::test]
async fn resource_url_auto_extracts_from_forwarded_headers_when_unset() {
    let mock_server = MockServer::start().await;
    let app = build_app_with_resource_override(&mock_server, None).await;
    ...
    assert_eq!(
        read_resource_url(response).await,
        "https://api.shop.com/weather?city=tokyo"
    );
}

#[tokio::test]
async fn resource_url_falls_back_to_host_header_without_x_forwarded() {
    let mock_server = MockServer::start().await;
    let app = build_app_with_resource_override(&mock_server, None).await;
