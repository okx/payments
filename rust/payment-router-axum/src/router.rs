...
pub enum BuildError {
    ...
    #[error("adapter prepare() failed: {0}")]
    AdapterPrepareFailed(String),
}
...
impl CompiledRouter {
    ...
    /// Return all registered `(route_key, UnifiedRouteConfig)` pairs in
    /// the original declaration order. Used by adapter `prepare()` hooks
    /// that need to walk the full route set (e.g. x402 builds its
    /// `RoutesConfig` from these).
    pub fn entries(&self) -> Vec<(String, UnifiedRouteConfig)> {
        self.routes
            .iter()
            .map(|r| (r.route_key.clone(), r.config.clone()))
            .collect()
    }
...
mod tests {
    ...
    fn cfg_with(adapters: &[&str]) -> UnifiedRouteConfig {
        let mut b = UnifiedRouteConfig::builder();
        for a in adapters {
            b = b.adapter(*a, DummyCfg);
        }
        b.build()
    }
