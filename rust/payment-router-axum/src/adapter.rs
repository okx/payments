...
//!    recovery preserved natively. For MPP this is a thin wrapper that
//!    dispatches to `EvmMpp::verify_charge` / `verify_session` based on
//!    the credential intent → `inner` → appends `Payment-Receipt`.
...
pub trait ProtocolAdapter: Send + Sync + 'static {
    ...
    ///
    /// `route_cfg` is the adapter-specific slice of
    /// `UnifiedRouteConfig::adapter_configs` — a type-erased wrapper.
    /// Each adapter recovers its concrete config type via
    /// `route_cfg.downcast_ref::<MyRouteConfig>()`.
    ///
    /// Returns:
    ...
    /// middleware (x402-axum `PaymentMiddleware`, mpp-evm `EvmMpp`) —
    /// spec §1 principle "Adapter is a thin shell".
    fn make_service(&self, inner: InnerService) -> InnerService;

    /// Optional hook called by [`crate::PaymentRouterLayer::new`] exactly
    /// once after all unified routes are known. Adapters whose internal
    /// middleware needs cross-route state (e.g. x402's
    /// `PaymentMiddleware` requires a flat `RoutesConfig` at
    /// construction) materialize that here.
    ///
    /// Default: no-op. Implementations should be idempotent only in the
    /// sense that they tolerate being called once; the layer never calls
    /// `prepare` more than once.
    fn prepare(
        &self,
        _routes: &[(String, crate::types::UnifiedRouteConfig)],
    ) -> Result<(), String> {
        Ok(())
    }

    /// Optional hook called once per request, after `detect` claimed it but
    /// before forwarding to the adapter's wrapped service. Adapters whose
    /// inner middleware can re-do path matching (and risk diverging from
    /// the outer router's match) should use this to pin the already-matched
    /// per-route cfg into `extensions` so the inner middleware can use it
    /// directly and skip its own lookup.
    ///
    /// Default: no-op. Used by [`crate::adapters::X402Adapter`] to inject
    /// `x402_axum::PreMatchedRoute`, closing the pattern-key bypass where
    /// x402's exact-string lookup would otherwise miss a route the outer
    /// router accepted via `:param` / `*` patterns.
    fn enrich_request_extensions(
        &self,
        _extensions: &mut http::Extensions,
        _route_cfg: &crate::types::UnifiedRouteConfig,
    ) {
    }
}
