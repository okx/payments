...
impl PaymentRouterLayer {
    ...
    /// references an unregistered adapter.name(), or if any adapter's
    /// `prepare()` hook reports an error (e.g. malformed per-adapter config).
    pub fn new(config: PaymentRouterConfig) -> Result<Self, BuildError> {
        ...
        // Give each adapter a chance to finalize its internal state now
        // that all routes are known. x402 uses this hook to build its
        // RoutesConfig from per-route adapter configs; MPP and others
        // get the default no-op.
        let entries = router.entries();
        for adapter in &adapters {
            adapter
                .prepare(&entries)
                .map_err(BuildError::AdapterPrepareFailed)?;
        }
        Ok(Self {
...
async fn dispatch(
    ...
    match matched {
        Some(i) => {
            // Let the matched adapter pin its per-route cfg into request
            // extensions BEFORE we forward — this is how an adapter whose
            // inner middleware would otherwise re-do path matching (e.g.
            // x402's `PaymentMiddleware`) gets the router-resolved cfg
            // without going through a lossy second match.
            state.adapters[i].enrich_request_extensions(&mut parts.extensions, &route_cfg);
...
mod tests {
    ...
    fn cfg_both() -> UnifiedRouteConfig {
        UnifiedRouteConfig::builder()
            .adapter("mpp", DummyCfg)
            .adapter("x402", DummyCfg)
            .build()
    }
