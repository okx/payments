...
mod tests {
    ...
    /// Placeholder typed config used by the stub adapter in this file's
    /// tests. The stub adapter ignores the config; we just need
    /// `AdapterConfig::new(...)` to accept a value.
    #[derive(Debug, Clone)]
    struct DummyCfg;
