...
//!
//! Single source of truth for the typed structs and the domain. Both the
//! on-chain Voucher signature (verified locally by
//! `voucher::verify_voucher`) and the payee-side SettleAuthorization /
//! CloseAuthorization signatures (produced locally by
//! `authorization::sign_*`) live here so the wire format cannot drift
