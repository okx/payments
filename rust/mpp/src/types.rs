...
/// Default `EvmPaymentChannel` escrow contract on X Layer mainnet.
///
/// Applied automatically when no `method_details` is set on the
/// `EvmSessionMethod` builder. Must be overridden via
/// [`crate::EvmSessionMethod::with_escrow`] (or `with_typed_method_details`)
/// when targeting a testnet / forked deployment — otherwise voucher
/// signatures will not verify against the real on-chain contract.
...
pub struct ChargeMethodDetails {
    ...
    /// Endpoint URL this charge protects (e.g. `https://api.shop.com/photo`).
    /// Forwarded as-is through `challenge.request` to SA API so merchants
    /// can aggregate transaction volume / revenue by URL. Merchant-owned:
    /// SDK does not auto-extract; pass the value when constructing the
    /// challenge yourself. Session mode does not support this field by
    /// design (one session may span multiple URLs).
    #[serde(skip_serializing_if = "Option::is_none")]
...
mod tests {
    ...
    #[test]
    fn charge_method_details_resource_url_serializes_when_set() {
        let d = ChargeMethodDetails {
            chain_id: 196,
            fee_payer: None,
            permit2_address: None,
            memo: None,
            splits: None,
            resource_url: Some("https://api.shop.com/photo?id=42".into()),
        };
        let json = serde_json::to_value(&d).unwrap();
        assert_eq!(json["resourceUrl"], "https://api.shop.com/photo?id=42");
    }
