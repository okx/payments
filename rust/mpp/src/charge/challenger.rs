...
//!     splits: None,
//!     resource_url: Some("https://api.shop.com/photo".into()),
//! });
...
pub struct EvmChargeChallengerConfig {
    ...
    /// Endpoint URL this challenger protects (e.g.
    /// `https://api.shop.com/photo`). Forwarded as
    /// `ChargeMethodDetails.resource_url` so SA can aggregate transaction
    /// volume / revenue by URL. Service-level: typically one challenger
    /// per endpoint. `None` disables URL reporting.
...
struct Inner {
    ...
    /// Per-endpoint URL for revenue attribution. `None` = disabled.
...
impl EvmChargeChallengerBuilder {
    ...
    /// Per-endpoint URL forwarded as `ChargeMethodDetails.resource_url`
    /// for revenue attribution. Typically one challenger per endpoint.
    pub fn resource_url(mut self, v: impl Into<String>) -> Self {
...
impl ChargeChallenger for EvmChargeChallenger {
    ...
    fn challenge(
        ...
        let details = ChargeMethodDetails {
            ...
            resource_url: self.inner.resource_url.clone(),
        };
        let request =
