...
use super::decode::{
    decode_challenge_request_recipient, extract_payer_and_signer, extract_required_str,
    parse_address, parse_b256, parse_optional_hex_bytes, parse_u128_default_zero, parse_u128_str,
...
impl EvmSessionMethod {
    pub(super) async fn handle_open(
        ...
        // 2. Resolve method_details for chain_id / escrow_contract /
        //    min_voucher_delta. Prefer the credential-echoed request — that's
        //    what the client signed against. `self.method_details` is only a
        //    fallback for clients that omit the field. (See
        //    `EvmSessionMethod::resolve_method_details` for the rationale —
        //    upstream `tempoxyz/mpp-rs` does the same thing.)
        let method_details = self.resolve_method_details(request)?;
