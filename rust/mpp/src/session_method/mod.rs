...
use crate::types::{
    ...
    DEFAULT_CHAIN_ID, DEFAULT_ESCROW_CONTRACT,
};
...
/// Default `method_details` = X Layer mainnet (`DEFAULT_CHAIN_ID` /
/// `DEFAULT_ESCROW_CONTRACT`). Override with `with_escrow` or
/// `with_typed_method_details` when targeting testnet / forks.
fn default_method_details() -> serde_json::Value {
    serde_json::to_value(SessionMethodDetails {
        chain_id: DEFAULT_CHAIN_ID,
        escrow_contract: DEFAULT_ESCROW_CONTRACT.into(),
        channel_id: None,
        min_voucher_delta: None,
        fee_payer: None,
        splits: None,
    })
    .expect("SessionMethodDetails serializes to JSON")
}
...
impl EvmSessionMethod {
    ...
    pub fn with_store(sa_client: Arc<dyn SaApiClient>, store: Arc<dyn SessionStore>) -> Self {
        Self {
            sa_client,
            store,
            method_details: Some(default_method_details()),
            signer: None,
            payee_address: None,
    ...
    /// Read-only accessor for the configured `methodDetails`. Used by
    /// `EvmMpp::session_challenge` to embed the method details into the
    /// session `PaymentChallenge`. Returns the default X Layer mainnet
    /// values when no override has been set.
    pub fn method_details_value(&self) -> Option<&serde_json::Value> {
    ...
    /// Minimal builder: override only the `escrow_contract` field on the
    /// stored `method_details`, preserving everything else
    /// (`chain_id` / `min_voucher_delta` / `fee_payer` / `splits` / `channel_id`).
    ///
    /// Use this to override the built-in [`DEFAULT_ESCROW_CONTRACT`] when
    /// targeting a testnet / forked deployment. If you're on X Layer mainnet
    /// against the canonical OKX `EvmPaymentChannel`, you can omit this call
    /// entirely — `with_store` already pre-fills the default.
    ///
    /// **Idempotent for the other fields**: composing with
    /// [`with_typed_method_details`] is order-independent — you can call
    /// `with_typed_method_details(...).with_escrow(...)` or the reverse and
    /// end up with the same combined state. (Earlier revisions of this
    /// method silently reset `chain_id` back to `DEFAULT_CHAIN_ID`, which
    /// broke testnet examples whose `with_escrow` call landed after the
    /// `chain_id` setter and caused EIP-712 voucher domains to drift between
    /// client and server.)
    ///
    /// # Panics
    ///
    /// Fail-fast on invalid address: a typo would otherwise surface as a
    /// `70000` from the first `handle_open` and break every voucher
    /// signature for the process lifetime. Validate inputs before calling.
    pub fn with_escrow(mut self, escrow_contract: impl Into<String>) -> Self {
        let escrow = escrow_contract.into();
        if let Err(e) = decode::parse_address(&escrow) {
            panic!("with_escrow: invalid address {escrow:?}: {e}");
        }
        // Decode existing method_details (or default) and mutate only the
        // `escrow_contract` field — never reset `chain_id` etc.
        let mut details: SessionMethodDetails = self
            .method_details
            .as_ref()
            .and_then(|v| serde_json::from_value(v.clone()).ok())
            .unwrap_or_else(|| SessionMethodDetails {
                chain_id: DEFAULT_CHAIN_ID,
                escrow_contract: DEFAULT_ESCROW_CONTRACT.into(),
                channel_id: None,
                min_voucher_delta: None,
        ...
        details.escrow_contract = escrow;
        self.method_details = Some(
            serde_json::to_value(&details).expect("SessionMethodDetails serializes to JSON"),
        );
        self
    }

    /// Resolve effective `method_details` for credential verification.
    ///
    /// Priority:
    /// 1. `request.method_details` — what the client signed against. This
    ///    is the authoritative value during verify, since the EIP-712
    ///    voucher domain (`chainId` / `verifyingContract`) was bound to it
    ///    when the client signed.
    /// 2. `self.method_details` — the server's configured fallback, used
    ///    when the credential omits the field (legacy / minimal clients).
    ///
    /// Matches upstream `tempoxyz/mpp-rs` `TempoSessionMethod::resolve_method_details`:
    /// the server reads what the client signed, never overriding silently
    /// with its own config. Without this, a server whose
    /// `self.method_details.chain_id` drifts from the outbound challenge's
    /// `chainId` (e.g. via a builder ordering footgun) will compute the
    /// EIP-712 domain with one chain id while the client signed with
    /// another → `ecrecover` returns garbage and every voucher fails as
    /// `signer mismatch`.
    pub(super) fn resolve_method_details(
        &self,
        request: &SessionRequest,
    ) -> Result<SessionMethodDetails, SaApiError> {
        if let Some(v) = request.method_details.as_ref() {
            return serde_json::from_value(v.clone()).map_err(|e| {
                SaApiError::new(70000, format!("invalid request.methodDetails: {e}"))
            });
        }
        decode::decode_method_details(self.method_details.as_ref())
    }
    ...
    ///
    /// Merchant code can call this outside of `verify_session` (e.g.
    /// non-linear billing tied to actual service consumption); the
    /// `voucher` action already calls it internally with
    ...
    pub(super) async fn close_with_authorization_locked(
        ...
        // Guard: refuse to close at a cum below the local highest. Same
        // 70012 invariant as `submit_voucher`; close is more sensitive
        // than voucher (settles on-chain), so it must be at least as
        // strict. When `cumulative_amount = None` this passes naturally
        // because `cumulative` defaults to `highest_voucher_amount`.
...
mod tests {
    ...
    /// No `.with_escrow()` call → builder leaves the X Layer mainnet
    /// default pre-filled by `with_store`. Decodes back into a typed
    /// `SessionMethodDetails` matching `DEFAULT_*` constants.
    #[test]
    fn default_method_details_targets_xlayer_mainnet() {
        let method = EvmSessionMethod::new(Arc::new(StubSa::default()));
        let raw = method.method_details.expect("default pre-filled");
        let parsed: SessionMethodDetails = serde_json::from_value(raw).unwrap();
        assert_eq!(parsed.chain_id, DEFAULT_CHAIN_ID);
        assert_eq!(
            parsed.escrow_contract.to_lowercase(),
            DEFAULT_ESCROW_CONTRACT.to_lowercase()
        );
    }

    /// `.with_escrow(DEFAULT_ESCROW_CONTRACT)` should be a valid no-op
    /// override (developers reading from config can pass the constant
    /// explicitly without panicking on the eager parse).
    #[test]
    fn with_escrow_accepts_default_constant() {
        let _ = EvmSessionMethod::new(Arc::new(StubSa::default()))
            .with_escrow(DEFAULT_ESCROW_CONTRACT);
    }

    /// Empty string is rejected at construction time, not deferred to
    /// `handle_open`. This is the footgun fix for users reading
    /// `cfg.escrow` from env where the env var may be unset.
    #[test]
    #[should_panic(expected = "with_escrow: invalid address")]
    fn with_escrow_empty_string_panics_at_build_time() {
        let _ = EvmSessionMethod::new(Arc::new(StubSa::default())).with_escrow("");
    }

    /// Malformed address (wrong length, not 0x-prefixed, etc.) is also
    /// rejected eagerly.
    #[test]
    #[should_panic(expected = "with_escrow: invalid address")]
    fn with_escrow_malformed_address_panics_at_build_time() {
        let _ = EvmSessionMethod::new(Arc::new(StubSa::default())).with_escrow("0xnope");
    }

    /// Regression: `with_escrow(...)` after `with_typed_method_details(...)`
    /// must preserve `chain_id` / `min_voucher_delta` / `fee_payer`. The
    /// previous implementation reset every non-escrow field back to the
    /// X Layer mainnet defaults, which produced a silent client/server
    /// EIP-712 domain mismatch on any non-default chain.
    #[test]
    fn with_escrow_preserves_other_method_details_fields() {
        let testnet_escrow = "0x5e550002e64faf79b41d89fe8439eeb1be66ce3b";
        let method = EvmSessionMethod::new(Arc::new(StubSa::default()))
            .with_typed_method_details(SessionMethodDetails {
                chain_id: 1952,
                escrow_contract: "0x0000000000000000000000000000000000000001".into(),
                channel_id: None,
                min_voucher_delta: Some("42".into()),
                fee_payer: Some(true),
                splits: None,
            })
            .expect("typed method details accepted")
            .with_escrow(testnet_escrow);
        let raw = method.method_details.expect("method_details set");
        let parsed: SessionMethodDetails = serde_json::from_value(raw).unwrap();
        assert_eq!(parsed.chain_id, 1952, "chain_id must survive with_escrow");
        assert_eq!(parsed.escrow_contract.to_lowercase(), testnet_escrow);
        assert_eq!(parsed.min_voucher_delta.as_deref(), Some("42"));
        assert_eq!(parsed.fee_payer, Some(true));
    }

    // ===================== resolve_method_details =====================

    /// Build a `SessionRequest` carrying a custom `methodDetails` payload —
    /// used to drive `resolve_method_details` priority tests.
    fn request_with_method_details(details: SessionMethodDetails) -> SessionRequest {
        let mut req = SessionRequest::default();
        req.method_details = Some(serde_json::to_value(details).unwrap());
        req
    }

    /// Verify priority: when the credential-echoed `request.methodDetails`
    /// has a value, the resolver returns it — regardless of what
    /// `self.method_details` says. This is what aligns server-side verify
    /// with what the client actually signed against.
    #[test]
    fn resolve_method_details_prefers_request() {
        let method = EvmSessionMethod::new(Arc::new(StubSa::default()))
            .with_typed_method_details(SessionMethodDetails {
                chain_id: DEFAULT_CHAIN_ID, // 196
                escrow_contract: DEFAULT_ESCROW_CONTRACT.into(),
                channel_id: None,
                min_voucher_delta: None,
                fee_payer: None,
            ...
            .unwrap();
        let request = request_with_method_details(SessionMethodDetails {
            chain_id: 1952, // testnet — must win over self's 196
            escrow_contract: "0x000000000000000000000000000000000000abcd".into(),
            channel_id: None,
            min_voucher_delta: Some("7".into()),
            fee_payer: Some(false),
            splits: None,
        });
        let resolved = method.resolve_method_details(&request).unwrap();
        assert_eq!(resolved.chain_id, 1952);
        assert_eq!(
            resolved.escrow_contract.to_lowercase(),
            "0x000000000000000000000000000000000000abcd"
        );
        assert_eq!(resolved.min_voucher_delta.as_deref(), Some("7"));
        assert_eq!(resolved.fee_payer, Some(false));
    }

    /// Fallback path: when `request.methodDetails` is absent, the resolver
    /// reads from `self.method_details`. Legacy / minimal clients that omit
    /// the field still verify against the server's configured defaults.
    #[test]
    fn resolve_method_details_falls_back_to_self() {
        let method = EvmSessionMethod::new(Arc::new(StubSa::default()))
            .with_typed_method_details(SessionMethodDetails {
                chain_id: 1952,
                escrow_contract: "0x000000000000000000000000000000000000beef".into(),
                channel_id: None,
                min_voucher_delta: None,
                fee_payer: None,
                splits: None,
            ...
            .unwrap();
        let request = SessionRequest::default(); // method_details = None
        let resolved = method.resolve_method_details(&request).unwrap();
        assert_eq!(resolved.chain_id, 1952);
        assert_eq!(
            resolved.escrow_contract.to_lowercase(),
