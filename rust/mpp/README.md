...
Published as `okxweb3-app-mpp`; `use` path is `mpp_evm::*`.
...
// X Layer mainnet defaults are applied automatically (chain_id = 196,
// escrow = DEFAULT_ESCROW_CONTRACT). Call `.with_escrow(...)` to override
// when targeting a testnet / forked deployment.
let session = Arc::new(EvmSessionMethod::new(sa));
