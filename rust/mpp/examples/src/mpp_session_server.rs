...
//! # MPP_ESCROW optional — omit to use X Layer mainnet default
//! # (DEFAULT_ESCROW_CONTRACT); set when targeting testnet / fork.
...
use mpp_evm::{CredentialExt, EvmSessionMethod, OkxSaApiClient, DEFAULT_ESCROW_CONTRACT};
    ...
    let cfg = match load_config() {
        ...
        Err(missing) => {
            ...
            eprintln!(
                ...
                MPP_MERCHANT_PRIVATE_KEY (MPP_ESCROW optional, defaults to X Layer mainnet)"
            );
            std::process::exit(1);
        }
    };
...
fn load_config() -> Result<Config, &'static str> {
    Ok(Config {
        ...
        // MPP_ESCROW is optional: omit to use the X Layer mainnet default
        // (`DEFAULT_ESCROW_CONTRACT`); set explicitly when targeting a
        // testnet / forked deployment.
        escrow: std::env::var("MPP_ESCROW").unwrap_or_else(|_| DEFAULT_ESCROW_CONTRACT.into()),
    })
}
