//! MPP EVM Payment-Gated Photo Server.
//!
//! Uses upstream `MppCharge<C>` extractor + `WithReceipt<T>` response
//! wrapper + `EvmChargeChallenger`, which eliminates the parse / verify /
//! format_www_authenticate boilerplate.
//!
//! Flow:
...
//!    header + photo URL
//!
//! # Per-endpoint URL reporting
//!
//! `resource_url` on `EvmChargeChallengerConfig` is a service-level
//! constant — typically the public URL of this endpoint
//! (`https://api.shop.com/photo`). SA aggregates revenue / transaction
//! volume per `resource_url`. Different endpoints → different
//! challenger instances, each with its own URL. Session mode does not
//! support this field by design (one session may span multiple URLs).
//!
//! # Running (real SA API)
//!
//! ```bash
...
async fn main() {
    ...
    let challenger: Arc<dyn ChargeChallenger> =
        Arc::new(EvmChargeChallenger::new(EvmChargeChallengerConfig {
            ...
            splits: None,
            // Public URL of this endpoint — SA aggregates revenue per
            // `resource_url`. Set per-challenger (one challenger per
            // endpoint). `None` disables URL reporting. Override via
            // `MPP_RESOURCE_URL`; falls back to a demo placeholder so
            // the example runs end-to-end out of the box.
            resource_url: Some(env.resource_url.clone()),
        }));
    ...
    println!("Resource URL: {}", env.resource_url);
...
struct Env {
    ...
    currency: String,
    recipient: String,
    /// Optional via env (`MPP_RESOURCE_URL`); defaults to a demo placeholder.
    resource_url: String,
}
...
fn load_env() -> Env {
    ...
    Env {
        ...
        resource_url: std::env::var("MPP_RESOURCE_URL")
            .unwrap_or_else(|_| "https://api.shop.com/photo".into()),
    }
}
