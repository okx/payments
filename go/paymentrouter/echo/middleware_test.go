...
func serve(mw echofw.MiddlewareFunc, req *http.Request) *httptest.ResponseRecorder {
	w := httptest.NewRecorder()
	e := echofw.New()
	e.GET("/api/weather", func(c echofw.Context) error {
...
func TestFor_NoDetect_Returns402(t *testing.T) {
	...
	gate := precho.New([]pr.ProtocolAdapter{
...
func TestFor_DetectHit_CommittedAborts(t *testing.T) {
	gate := precho.New([]pr.ProtocolAdapter{
