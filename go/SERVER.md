...
requirements, _ := server.BuildPaymentRequirements(ctx, config, supportedKind, extensions)

// Verify payment
verifyResult, _ := server.VerifyPayment(ctx, payload, requirements)

// Settle payment (overrides can be nil)
settleResult, _ := server.SettlePayment(ctx, payload, requirements, nil)
```
...
// Handle settlement (overrides can be nil)
result := httpServer.ProcessSettlement(ctx, payload, requirements, nil)
```
...
```go
import ginmw "github.com/okx/payments/go/http/gin"
...
- `SyncFacilitatorOnStart` - Query facilitator on startup (default: true)
- `Timeout` - Context timeout for operations (default: 30s)
...
server.OnAfterSettle(func(ctx x402.SettleResultContext) error {
    log.Printf("Payment settled: tx=%s", ctx.Result.Transaction)
    return nil
})
```
...
import (
    ...
    "github.com/okx/payments/go/extensions/types"
)
...
**Payment Methods:**
```go
func (s *X402ResourceServer) BuildPaymentRequirements(ctx context.Context, config ResourceConfig, supportedKind SupportedKind, extensions []string) (PaymentRequirements, error)
...
func (s *X402ResourceServer) SettlePayment(ctx context.Context, payload PaymentPayload, requirements PaymentRequirements, overrides *SettlementOverrides) (*SettleResponse, error)
```
...
type RouteConfig struct {
    Accepts            PaymentOptions             // Payment options for this route
    Resource           string                     // HTTP resource identifier
    Description        string                     // Resource description
    MimeType           string                     // Response content type
    CustomPaywallHTML  string                     // Custom HTML for browser paywall
    Extensions         map[string]interface{}     // Protocol extensions
    UnpaidResponseBody UnpaidResponseBodyFunc     // Custom response body for 402
}
...
type PaymentOption struct {
    ...
    MaxTimeoutSeconds int                     // Max payment timeout
    Extra             map[string]interface{}  // Extension-specific data
}
```
...
type Config struct {
    ...
    SyncFacilitatorOnStart bool                              // Default: true
...
server.OnBeforeVerify(func(ctx VerifyContext) (*BeforeHookResult, error) {
    ...
    log.Printf("Verifying payment on %s", ctx.Requirements.GetNetwork())
    return nil, nil
})
...
server.OnBeforeSettle(func(ctx SettleContext) (*BeforeHookResult, error) {
    // Access payer from payload map
    payerAddr, _ := ctx.Payload.GetPayload()["from"].(string)
    if isBlacklisted(payerAddr) {
...
```go
facilitator := x402http.NewHTTPFacilitatorClient(&x402http.FacilitatorConfig{
    URL: "https://api.cdp.coinbase.com", // Coinbase production facilitator
})
```
...
**V1:**
```go
import "github.com/okx/payments/go/middleware/gin"
```
