...
import (
    ...
    evm "github.com/okx/payments/go/x402/mechanisms/evm/exact/facilitator"
)
...
func main() {
    ...
    r.GET("/supported", handleGetSupported(facilitator))
...
// Register payment mechanisms (first param is []Network slice)
facilitator.Register([]x402.Network{"eip155:84532"}, schemeFacilitator)

// Query supported networks/schemes (no context, no error)
...
facilitator.OnBeforeVerify(func(ctx FacilitatorVerifyContext) (*FacilitatorBeforeHookResult, error) {
    ...
    // return &FacilitatorBeforeHookResult{Abort: true, Reason: "..."}, nil
...
facilitator.OnVerifyFailure(func(ctx FacilitatorVerifyFailureContext) (*FacilitatorVerifyFailureHookResult, error) {
    ...
    // return &FacilitatorVerifyFailureHookResult{Recovered: true, Result: ...}, nil
...
facilitator.OnSettleFailure(func(ctx FacilitatorSettleFailureContext) (*FacilitatorSettleFailureHookResult, error) {
    ...
    //     return &FacilitatorSettleFailureHookResult{Recovered: true, Result: result}, nil
    // }
...
```go
facilitator.OnBeforeSettle(func(ctx FacilitatorSettleContext) (*FacilitatorBeforeHookResult, error) {
    payer, _ := ctx.Payload.GetPayload()["from"].(string)
...
facilitator.OnBeforeVerify(func(ctx FacilitatorVerifyContext) (*FacilitatorBeforeHookResult, error) {
    payer, _ := ctx.Payload.GetPayload()["from"].(string)
    if fraudDetector.IsSuspicious(payer) {
        return &FacilitatorBeforeHookResult{
            Abort: true,
...
```go
import svm "github.com/okx/payments/go/x402/mechanisms/svm"
...
func TestFacilitatorVerify(t *testing.T) {
    facilitator := x402.Newx402Facilitator()
    facilitator.Register([]x402.Network{network}, mockScheme)
...
facilitator := x402.Newx402Facilitator()
facilitator.Register([]x402.Network{network}, evm.NewExactEvmScheme(signer, nil))
...
facilitator.OnAfterSettle(func(ctx FacilitatorSettleResultContext) error {
    metrics.IncrementCounter("facilitator.settlements", map[string]string{
        "network": string(ctx.Result.Network),  // Network type → string
...
facilitator.
    Register([]x402.Network{"eip155:1"}, evm.NewExactEvmScheme(evmMainnetSigner, nil)).
    Register([]x402.Network{"eip155:84532"}, evm.NewExactEvmScheme(evmTestnetSigner, nil))
```
