...
import (
    ...
    evm "github.com/okx/payments/go/x402/mechanisms/evm/exact/client"
    evmsigners "github.com/okx/payments/go/x402/signers/evm"
)
...
func main() {
    ...
    client := x402.Newx402Client().
        Register("eip155:*", evm.NewExactEvmScheme(signer, nil))  // 2nd param is optional *ExactEvmSchemeConfig
...
```go
import evmsigners "github.com/okx/payments/go/x402/signers/evm"
...
```go
import svmsigners "github.com/okx/payments/go/x402/signers/svm"
...
```go
client := x402.Newx402Client()  // accepts optional ...ClientOption
...
// All Solana networks
client.Register("solana:*", svm.NewExactSvmScheme(svmSigner))  // config is variadic optional
```
...
```go
// Ethereum Mainnet
client.Register("eip155:1", evm.NewExactEvmScheme(mainnetSigner, nil))   // nil config = use defaults
...
client.OnBeforePaymentCreation(func(ctx x402.PaymentCreationContext) (*x402.BeforePaymentCreationHookResult, error) {
    ...
    // Can abort by returning: &BeforePaymentCreationHookResult{Abort: true, Reason: "..."}
...
client.OnAfterPaymentCreation(func(ctx x402.PaymentCreatedContext) error {
    ...
    fmt.Printf("Payment created for scheme: %s\n", ctx.Payload.GetScheme())
    return nil
})
...
client.OnPaymentCreationFailure(func(ctx x402.PaymentCreationFailureContext) (*x402.PaymentCreationFailureHookResult, error) {
    ...
    // Can recover by returning: &PaymentCreationFailureHookResult{Recovered: true, Payload: ...}
...
```go
client.OnAfterPaymentCreation(func(ctx PaymentCreatedContext) error {
    log.Printf("Payment created for %s: %s",
...
client.OnAfterPaymentCreation(func(ctx PaymentCreatedContext) error {
    metrics.IncrementCounter("payments.created", map[string]string{
        "network": ctx.SelectedRequirements.GetNetwork(),
        "scheme":  ctx.SelectedRequirements.GetScheme(),
    })
    return nil
})
```
...
```go
client.OnBeforePaymentCreation(func(ctx PaymentCreationContext) (*BeforePaymentCreationHookResult, error) {
    // Don't pay for resources over a certain price
    if exceedsLimit(ctx.SelectedRequirements.GetAmount()) {
        return &BeforePaymentCreationHookResult{
            Abort: true,
...
func (c *X402Client) RegisterV1(network Network, scheme SchemeNetworkClientV1) *X402Client
```
...
client.Register("eip155:1", evm.NewExactEvmScheme(signer, nil))
```
...
```go
client.Register("eip155:*", evm.NewExactEvmScheme(signer, nil))
```
...
client.Register("eip155:84532", evm.NewExactEvmScheme(signer, nil))
// OR
client.Register("eip155:*", evm.NewExactEvmScheme(signer, nil))
```
...
```go
client.
    Register("eip155:*", evm.NewExactEvmScheme(signer, nil)).     // V2
