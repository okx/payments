...
func serveExemptEcho(t *testing.T, exempt []string, sigValid bool, verifyErr error, from string) (*httptest.ResponseRecorder, *exemptCounters) {
	t.Helper()
	...
	e := echo.New()
	e.Use(PaymentMiddlewareFromConfig(routes, opts...))
...
func TestExempt_ServedWithoutVerifyOrSettle(t *testing.T) {
	w, calls := serveExemptEcho(t, []string{"0xReviewer"}, true, fmt.Errorf("insufficient_funds"), "0xReviewer")
	if w.Code != http.StatusOK {
...
func TestExempt_NonExemptTakesNormalPath(t *testing.T) {
	w, calls := serveExemptEcho(t, []string{"0xReviewer"}, true, nil, "0xSomeoneElse")
	if w.Code != http.StatusOK {
...
func TestExempt_InvalidSignatureFallsThrough(t *testing.T) {
	_, calls := serveExemptEcho(t, []string{"0xReviewer"}, false, nil, "0xReviewer")
...
func TestExempt_CaseInsensitiveMatch(t *testing.T) {
	w, calls := serveExemptEcho(t, []string{"0xABCdef"}, true, nil, "0xabcDEF")
