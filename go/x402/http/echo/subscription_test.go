...
// TestPeriodSubscribeSettlesWithRouteSyncSettle verifies the echo period subscribe
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			e.POST("/subscribe", func(c echo.Context) error {
				nextCalled = true
				return c.JSON(http.StatusOK, map[string]string{"data": "granted"})
			})
...
func TestSubscriptionCancelRelays(t *testing.T) {
	...
	e.POST("/cancel", func(c echo.Context) error {
		return c.JSON(http.StatusTeapot, map[string]string{"unexpected": "handler"})
	})
...
func TestSubscriptionCancelPendingRelays(t *testing.T) {
	...
	e.POST("/cancel-pending", func(c echo.Context) error {
		return c.JSON(http.StatusTeapot, map[string]string{"unexpected": "handler"})
	})
...
func TestSubscriptionChangeAccessDegradesToOffer(t *testing.T) {
	...
	e.POST("/subscription/change", func(c echo.Context) error {
		served = true
		return c.JSON(http.StatusOK, map[string]string{"data": "granted"})
	})
