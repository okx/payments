...
// SetSettlementOverrides sets settlement overrides on the Echo response for partial
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if server.SubscriptionEnabled() {
				subCtx, subCancel := context.WithTimeout(c.Request().Context(), config.Timeout)
				handled, err := handleSubscription(c, next, server, reqCtx, subCtx)
				subCancel()
				if handled {
					return err
				}
...
func handlePaymentVerified(c echo.Context, next echo.HandlerFunc, server *x402http.HTTPServer, ctx context.Context, result x402http.HTTPProcessResult, config *MiddlewareConfig) error {
	...
	if overridesHeader := origWriter.Header().Get(x402http.SettlementOverridesHeader); overridesHeader != "" {
		...
		origWriter.Header().Del(x402http.SettlementOverridesHeader)
	}
