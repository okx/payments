...
const (
	// DefaultPollInterval is the interval between settle/status polls.
	DefaultPollInterval = 1 * time.Second

	// DefaultPollDeadline is the maximum time to poll settle/status.
	DefaultPollDeadline = 5 * time.Second
)

// OnSettlementTimeoutHook is called when the facilitator returns status="timeout"
// and polling fails. It receives the tx hash and network so the server can verify on-chain.
...
// AddRoutes compiles and appends additional route configurations to the server.
// This can be called after construction to register routes dynamically.
// Returns the server for method chaining.
func (s *x402HTTPResourceServer) AddRoutes(routes RoutesConfig) *x402HTTPResourceServer {
	for pattern, config := range routes {
		verb, path, regex := parseRoutePattern(pattern)
		s.compiledRoutes = append(s.compiledRoutes, CompiledRoute{
			Verb:    verb,
			Regex:   regex,
			Config:  config,
			Pattern: path,
...
// SetPollDeadline sets the maximum time to poll settle/status on timeout recovery.
// Default is 5 seconds.
func (s *x402HTTPResourceServer) SetPollDeadline(deadline time.Duration) *x402HTTPResourceServer {
	s.pollDeadline = deadline
	return s
}

// OnSettlementTimeout registers a hook called when the facilitator returns status="timeout"
// and built-in polling fails. The hook should verify the tx on-chain and return confirmed=true
// to deliver the resource, or confirmed=false to return 402.
...
func (s *x402HTTPResourceServer) ProcessHTTPRequest(ctx context.Context, reqCtx HTTPRequestContext, paywallConfig *PaywallConfig) HTTPProcessResult {
	...
	// Fail closed — reject when result is nil or isValid is false.
	// Matches TS x402HTTPResourceServer behavior.
...
//
// When the facilitator returns status="timeout", ProcessSettlement attempts recovery:
//  1. Poll GET /settle/status at 1s intervals for up to pollDeadline (default 5s)
//  2. If polling fails, call the OnSettlementTimeout hook (if registered)
//  3. If both fail, return 402
func (s *x402HTTPResourceServer) ProcessSettlement(ctx context.Context, payload types.PaymentPayload, requirements types.PaymentRequirements, overrides *x402.SettlementOverrides) *ProcessSettleResult {
	...
	// Handle timeout: poll settle/status, then try timeout hook
	if settleResult.Status == "timeout" {
		recovered := false
		if settleResult.Transaction != "" {
			recovered = s.recoverFromTimeout(ctx, settleResult, requirements)
		}
		if !recovered {
...
// recoverFromTimeout attempts to recover from a settlement timeout.
// First polls settle/status, then falls back to the timeout hook.
func (s *x402HTTPResourceServer) recoverFromTimeout(ctx context.Context, settleResult *x402.SettleResponse, requirements types.PaymentRequirements) bool {
	// Step 1: Poll settle/status
	pollResult := s.pollSettleStatus(ctx, settleResult.Transaction, requirements)
	if pollResult == x402.PollResultSuccess {
		return true
	}

	// Step 2: Try timeout hook
	if s.timeoutRecoveryHook != nil {
		confirmed, err := s.timeoutRecoveryHook(ctx, settleResult.Transaction, string(settleResult.Network))
		if err != nil {
			log.Printf("[x402] onSettlementTimeout hook error: %v", err)
		}
		if confirmed {
			return true
		}
...
// pollSettleStatus polls GET /settle/status until a terminal state or deadline.
// Returns PollResultSuccess, PollResultFailed, or PollResultTimeout.
func (s *x402HTTPResourceServer) pollSettleStatus(ctx context.Context, txHash string, requirements types.PaymentRequirements) x402.PollResult {
	network := x402.Network(requirements.Network)
	scheme := requirements.Scheme

	s.X402ResourceServer.RLock()
	facilitator := s.X402ResourceServer.GetFacilitatorClient(network, scheme)
	s.X402ResourceServer.RUnlock()

	checker, ok := facilitator.(x402.SettleStatusChecker)
	if !ok || checker == nil {
		return x402.PollResultTimeout
	}

	deadline := s.pollDeadline
	if deadline == 0 {
	...
	for time.Now().Before(end) {
		resp, err := checker.GetSettleStatus(ctx, txHash)
		if err == nil {
			if !resp.Success {
				return x402.PollResultFailed
			}
			if resp.Status == "success" {
				return x402.PollResultSuccess
			}
