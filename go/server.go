...
// GetFacilitatorClient returns the facilitator client for a given network/scheme, or nil if not found.
...
func (s *x402ResourceServer) GetFacilitatorClient(network Network, scheme string) FacilitatorClient {
	...
	return networkClients[scheme]
}

// RLock acquires a read lock on the server's mutex.
func (s *x402ResourceServer) RLock() { s.mu.RLock() }

// RUnlock releases the read lock on the server's mutex.
func (s *x402ResourceServer) RUnlock() { s.mu.RUnlock() }
