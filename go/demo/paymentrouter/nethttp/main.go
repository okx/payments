...
// adapters on net/http.
//
// Usage:
//
//	go run -a ./paymentrouter/nethttp
//
// Endpoints:
//
...
func main() {
	...
	// ── net/http + PaymentGate ──
	mux := http.NewServeMux()

	mux.HandleFunc("GET /free", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(map[string]string{"message": "This endpoint is free!"}) //nolint:errcheck
	})

	paid := prhttp.New([]pr.ProtocolAdapter{mppAdapter, x402Adapter},
	...
	mux.Handle("GET /api/onetime", paid.For(onetimeCfg)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(map[string]string{"message": "One-time payment received!"}) //nolint:errcheck
	})))

	mux.Handle("GET /api/batch", paid.For(batchCfg)(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(map[string]string{"message": "Batch request served!"}) //nolint:errcheck
	})))

	port := envOrDefault("PORT", "8080")
	log.Printf("PaymentRouter nethttp demo listening on :%s", port)
