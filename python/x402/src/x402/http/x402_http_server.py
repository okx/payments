...
class x402HTTPResourceServer(x402HTTPServerBase):
    ...
    async def _recover_from_timeout_async(
        self,
        tx_hash: str,
        requirements: PaymentRequirements,
    ) -> bool:
        """Async timeout recovery: poll settle/status, then try hook."""
        from .x402_http_server_base import DEFAULT_POLL_DEADLINE, DEFAULT_POLL_INTERVAL

        poll_result = await self._poll_settle_status_async(tx_hash, requirements)
    ...
    async def _poll_settle_status_async(
        self,
        tx_hash: str,
        requirements: PaymentRequirements,
    ) -> str:
        """Async poll GET /settle/status until terminal state or deadline."""
        from .x402_http_server_base import DEFAULT_POLL_DEADLINE, DEFAULT_POLL_INTERVAL
        ...
        while time.monotonic() < end:
            try:
                result = client.get_settle_status(tx_hash)
