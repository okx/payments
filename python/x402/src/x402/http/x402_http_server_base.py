...
# ============================================================================
# Settlement Timeout Recovery Constants
# ============================================================================
...
class x402HTTPServerBase:
    ...
    def set_poll_deadline(self, deadline: float) -> x402HTTPServerBase:
        """Set the maximum time to poll settle/status on timeout recovery.

        Args:
            deadline: Maximum seconds to poll. Default is 5.0.
        """
        self._poll_deadline = deadline
        return self

    def on_settlement_timeout(self, hook: OnSettlementTimeoutHook) -> x402HTTPServerBase:
        """Register a hook called when the facilitator returns status="timeout"
        and built-in polling fails.

        The hook receives (tx_hash, network) and should return True to confirm
        the settlement (deliver the resource), or False to return 402.
        """
        self._timeout_recovery_hook = hook
        return self

    def add_routes(self, routes: RoutesConfig) -> None:
        """Add routes dynamically (used by PaymentRouter adapter)."""
        self._compile_routes(routes)
    ...
    def process_settlement(
        ...
        try:
            ...
            if settle_response.status == "timeout":
                recovered = False
                ...
                if not recovered:
                    ...
                    failure.response = self._build_settlement_failure_response(failure, context)
                    return failure
                settle_response.status = "success"
    ...
    def _recover_from_timeout(
        ...
        """Attempt to recover from a settlement timeout.

        Step 1: Poll GET /settle/status at 1s intervals up to poll_deadline.
        Step 2: If polling fails, call the timeout recovery hook (if registered).
        """
        # Step 1: Poll settle/status
        poll_result = self._poll_settle_status(tx_hash, requirements)
        if poll_result == "success":
            return True

        # Step 2: Try timeout hook
        if self._timeout_recovery_hook is not None:
            try:
                confirmed = self._timeout_recovery_hook(tx_hash, requirements.network)
                if confirmed:
                    return True
            except Exception as e:
                logger.error("[x402] onSettlementTimeout hook error: %s", e)

        return False

    def _poll_settle_status(
        self,
        tx_hash: str,
        requirements: PaymentRequirements,
    ) -> str:
        """Poll GET /settle/status until terminal state or deadline.
        ...
        client = self._server._facilitator_clients_map.get(network, {}).get(scheme)
        if client is None:
            return "timeout"

        # Check if client supports get_settle_status
        if not hasattr(client, "get_settle_status"):
            return "timeout"

        deadline = self._poll_deadline or DEFAULT_POLL_DEADLINE
        end = time.monotonic() + deadline

        while time.monotonic() < end:
            try:
                resp = client.get_settle_status(tx_hash)
                if not resp.success:
                    return "failed"
            ...
            if remaining <= 0:
                break
            sleep_time = min(DEFAULT_POLL_INTERVAL, remaining)
