...
class OKXFacilitatorClientTest {
    ...
    @Test
    void verifySignatureHappyPath() throws Exception {
        wm.stubFor(post(urlEqualTo("/api/v6/pay/x402/verify-signature"))
                .willReturn(aResponse()
                        .withHeader("Content-Type", "application/json")
                        .withBody("{\"isValid\":true,\"payer\":\"0xReviewWallet\"}")));

        PaymentPayload payload = new PaymentPayload();
        PaymentRequirements req = new PaymentRequirements();
        req.network = "eip155:196";

        VerifyResponse vr = client.verifySignature(payload, req);
        assertTrue(vr.isValid);
        assertEquals("0xReviewWallet", vr.payer);
    }
