...
public class OKXFacilitatorConfig {
    ...
    // Required. Package-private (not public) so the credentials are not
    // exposed to arbitrary code holding a config reference — only the
    // facilitator client in this package reads them. Set them via the
    // constructor.
    final String apiKey;
