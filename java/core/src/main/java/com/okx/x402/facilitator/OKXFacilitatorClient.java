...
public class OKXFacilitatorClient implements FacilitatorClient {
    ...
    private static final String VERIFY_SIGNATURE_PATH =
            "/api/v6/pay/x402/verify-signature";
    ...
    /**
     * Signature-only verification via {@code POST /verify-signature}:
     * authenticates the signer (signer == authorization.from) and nothing
     * else — no blacklist / KYS, no requirements matching, no time-window,
     * no on-chain balance / nonce / allowance checks, no nonce replay
     * protection. Supports exact (EIP-3009 ecrecover), aggr_deferred
     * (session-key Ed25519) and upto (Permit2).
     *
     * <p>The request body is identical to {@link #verify} (the facilitator
     * ignores {@code paymentRequirements} for this endpoint). A valid result
     * means "signature is authentic", NOT "safe to settle".
     */
    @Override
    public VerifyResponse verifySignature(PaymentPayload payload,
                                          PaymentRequirements requirements)
            throws IOException, InterruptedException {
        VerifyRequest req = new VerifyRequest();
        req.x402Version = 2;
        ...
        String body = Json.MAPPER.writeValueAsString(req);
        String responseBody = doPost(VERIFY_SIGNATURE_PATH, body);
        return Json.MAPPER.readValue(responseBody, VerifyResponse.class);
    }
