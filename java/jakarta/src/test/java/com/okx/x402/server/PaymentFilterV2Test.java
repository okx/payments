...
class PaymentFilterV2Test {
    ...
    // -----------------------------------------------------------------------
    // Exempt-payer (fee waiver) tests
    // -----------------------------------------------------------------------

    /** Payload whose EIP-3009 authorization.from is the given address. */
    private static PaymentPayload payloadWithFrom(String from) {
        PaymentPayload pp = new PaymentPayload();
        pp.x402Version = 2;
        pp.payload = Map.of(
                "signature", "0xabc",
                "authorization", Map.of("from", from, "to", "0xReceiver"));
        return pp;
    }

    @Test
    void exemptPayerSkipsVerifyAndSettle() throws Exception {
        when(req.getMethod()).thenReturn("GET");
        when(req.getRequestURI()).thenReturn("/protected/xlayer-usdt");
        when(req.getRequestURL()).thenReturn(new StringBuffer("http://localhost/protected/xlayer-usdt"));
        when(req.getHeader("PAYMENT-SIGNATURE"))
                .thenReturn(payloadWithFrom("0xOkxReviewWallet").toHeader());

        VerifyResponse sig = new VerifyResponse();
        sig.isValid = true;
        sig.payer = "0xOkxReviewWallet";
        when(fac.verifySignature(any(), any())).thenReturn(sig);
        when(resp.getStatus()).thenReturn(200);

        // Case-insensitive: configured lowercase, payload mixed-case.
        filter.processor().exemptPayers(java.util.List.of("0xokxreviewwallet"));

        filter.doFilter(req, resp, chain);

        // Resource served; verify and settle both skipped; no settlement header.
        verify(chain).doFilter(eq(req), any(jakarta.servlet.ServletResponse.class));
        verify(fac).verifySignature(any(), any());
        verify(fac, never()).verify(any(), any());
        verify(fac, never()).settle(any(), any(), anyBoolean());
        verify(resp, never()).setStatus(HttpServletResponse.SC_PAYMENT_REQUIRED);
        verify(resp, never()).setHeader(eq("PAYMENT-RESPONSE"), any());
    }

    @Test
    void exemptPayerInvalidSignatureFallsBackToNormalFlow() throws Exception {
        when(req.getMethod()).thenReturn("GET");
        when(req.getRequestURI()).thenReturn("/protected/xlayer-usdt");
        when(req.getRequestURL()).thenReturn(new StringBuffer("http://localhost/protected/xlayer-usdt"));
        when(req.getHeader("PAYMENT-SIGNATURE"))
        ...
        // Forged claim: whitelist address in `from` but signature doesn't match.
        VerifyResponse sig = new VerifyResponse();
        sig.isValid = false;
        when(fac.verifySignature(any(), any())).thenReturn(sig);
        ...
        // Falls through to the normal verify → settle flow (user pays).
        verify(fac).verify(any(), any());
        verify(fac).settle(any(), any(), anyBoolean());
    }

    @Test
    void exemptPayerEndpointFailureFallsBackToNormalFlow() throws Exception {
        when(req.getMethod()).thenReturn("GET");
        when(req.getRequestURI()).thenReturn("/protected/xlayer-usdt");
        when(req.getRequestURL()).thenReturn(new StringBuffer("http://localhost/protected/xlayer-usdt"));
        when(req.getHeader("PAYMENT-SIGNATURE"))
        ...
        when(fac.verifySignature(any(), any()))
                .thenThrow(new IOException("signature endpoint down"));
    ...
    @Test
    void nonExemptPayerNeverHitsSignatureEndpoint() throws Exception {
        when(req.getMethod()).thenReturn("GET");
        when(req.getRequestURI()).thenReturn("/protected/xlayer-usdt");
        when(req.getRequestURL()).thenReturn(new StringBuffer("http://localhost/protected/xlayer-usdt"));
        when(req.getHeader("PAYMENT-SIGNATURE"))
                .thenReturn(payloadWithFrom("0xRegularUser").toHeader());
        ...
        verify(fac, never()).verifySignature(any(), any());
        verify(fac).verify(any(), any());
        verify(fac).settle(any(), any(), anyBoolean());
    }

    @Test
    void emptyExemptListDisablesFeature() throws Exception {
        when(req.getMethod()).thenReturn("GET");
        when(req.getRequestURI()).thenReturn("/protected/xlayer-usdt");
        when(req.getRequestURL()).thenReturn(new StringBuffer("http://localhost/protected/xlayer-usdt"));
        when(req.getHeader("PAYMENT-SIGNATURE"))
        ...
        // Default: no whitelist configured — nothing changes.
        filter.doFilter(req, resp, chain);
    ...
    @Test
    void exemptPayerPermit2AuthorizationFrom() throws Exception {
        when(req.getMethod()).thenReturn("GET");
        when(req.getRequestURI()).thenReturn("/protected/xlayer-usdt");
        when(req.getRequestURL()).thenReturn(new StringBuffer("http://localhost/protected/xlayer-usdt"));
        ...
        pp.payload = Map.of(
                "signature", "0xabc",
                "permit2Authorization", Map.of("from", "0xOkxReviewWallet"));
        when(req.getHeader("PAYMENT-SIGNATURE")).thenReturn(pp.toHeader());
    ...
    @Test
    void exemptUnknownSchemeShapeFallsBackToNormalFlow() throws Exception {
        when(req.getMethod()).thenReturn("GET");
        when(req.getRequestURI()).thenReturn("/protected/xlayer-usdt");
        when(req.getRequestURL()).thenReturn(new StringBuffer("http://localhost/protected/xlayer-usdt"));

        // No authorization / permit2Authorization — `from` unreadable.
        PaymentPayload pp = new PaymentPayload();
        pp.x402Version = 2;
        pp.payload = Map.of("signature", "0xabc");
        when(req.getHeader("PAYMENT-SIGNATURE")).thenReturn(pp.toHeader());
